...
- La base de données SQLite (`dm_info.db`) est créée automatiquement au premier démarrage
- Quand vous ouvrez la page, elle initialise la base de données avec 5 utilisateurs par défaut
- Quand vous entrez votre nom et email, un nouvel utilisateur est créé dans la base de données
- Les messages sont stockés dans la base de données et persistent entre les sessions
